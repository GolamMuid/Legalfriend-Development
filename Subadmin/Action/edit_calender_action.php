<?php
    ...
    $result=mysqli_query($conn,"SELECT count(*) as total from calenders");
    $data=mysqli_fetch_assoc($result);
    if($data['total']>=1){
      
        if($_FILES["logo"]["name"]!=""){
            $update = "UPDATE calenders SET image = '$site_logo' WHERE id = $id;";
                mysqli_query($conn,$update);
        }
    
         if($_POST['image_title']!=""){
            $update = "UPDATE calenders SET image_link = '$image_title' WHERE id = $id;";
                mysqli_query($conn,$update);
        }
