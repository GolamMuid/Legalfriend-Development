<?php
    ...
    $result=mysqli_query($conn,"SELECT count(*) as total from videos");
    $data=mysqli_fetch_assoc($result);
    if($data['total']>=1){
      

        if($_POST['notification_headline']!=""){
            $update = "UPDATE videos SET headline = '$headline' WHERE id = $id;";
                mysqli_query($conn,$update);
        }
        
         if($_POST['image_title']["name"]!=""){
            $update = "UPDATE videos SET image_link = '$image_title' WHERE id = $id;";
                mysqli_query($conn,$update);
        }
